...
def setup():
    ...
    mMeter1 = rm.open_resource('USB0::0x1AB1::0x09C4::DM3R193601828::INSTR')
    mMeter2 = rm.open_resource('USB0::0x1AB1::0x09C4::DM3R193601838::INSTR')
    pSupply = rm.open_resource('USB0::0x1AB1::0x0E11::DP8C192803287::INSTR')
...
def main():
    ...
    while(cycle < 1):
        ...
        if(float(v) < 3.67):  #if charge low then charge
            filepath =  "C:\\Users\\bwhitehe\\Desktop\\BatteryCharacterizerPyVISA-master\\{0}_Data\\batC{1}.csv".format(sys.argv[1],cycle+1)
            f = open(filepath, "w")
            print("here")
            ...
            filepath =  "C:\\Users\\bwhitehe\\Desktop\\BatteryCharacterizerPyVISA-master\\{0}_Data\\batD{1}.csv".format(sys.argv[1],cycle+1)
            f = open(filepath, "w")
            f.write("Time,Volt,Current\n")
            eLoadSetup(eLoad);
            start_time = time.time()
